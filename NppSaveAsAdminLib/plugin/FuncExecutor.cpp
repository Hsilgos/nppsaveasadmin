  ...
  OutData &out_data)
{
  ...
  PacketWithCommand<InData> *in_ptr = prepareVectorToStoreData<PacketWithCommand<InData>>(in_buffer);
								 ...
								 DWORD flags_and_attributes)
{
	...
	if( !execute_function(pipe_sender, pipe_receiver, tcfdw, CreateFileWCmd, result) )
  ...
  do
	{
		...
		twfd.num_bytes_to_write = min(WriteFileData::MaxBufferSize, number_of_bytes_to_write);
    if (ptr)
    ...
    if (!execute_function(pipe_sender, pipe_receiver, twfd, WriteFileCmd, result))
    {
...
DWORD execute_get_file_type(Pipe &pipe_sender, Pipe &pipe_receiver, HANDLE handle)
{
	GetFileTypeData tgft = {0};
	tgft.handle = handle;
	GetFileTypeResult result	= {0};
	if( !execute_function(pipe_sender, pipe_receiver, tgft, GetFileTypeCmd, result) )
		return FALSE;
...
BOOL execute_close_handle(Pipe &pipe_sender, Pipe &pipe_receiver, HANDLE handle)
{
	CloseHandleData tchd = {0};
	tchd.handle = handle;
	CloseHandleResult result	= {0};
	if(!execute_function(pipe_sender, pipe_receiver, tchd, CloseHandleCmd, result) )
		return FALSE;
