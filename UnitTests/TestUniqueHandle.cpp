...
TEST_F(TestUniqueHandle, CustomDeleterAndValidHandle) {
	...
	UniqueHandle<std::function<void(HANDLE)>> unique_handle(ValidHandle, deleter.AsStdFunction());
...
TEST_F(TestUniqueHandle, CustomDeleterAndInvalidHandle) {
	...
	UniqueHandle<std::function<void(HANDLE)>> unique_handle(INVALID_HANDLE_VALUE, deleter.AsStdFunction());
	EXPECT_FALSE(unique_handle);
	EXPECT_EQ(INVALID_HANDLE_VALUE, unique_handle.get());
	EXPECT_EQ(INVALID_HANDLE_VALUE, *unique_handle);
...
TEST_F(TestUniqueHandle, MoveCtorFromValid) {
	...
	UniqueHandle<std::function<void(HANDLE)>> unique_handle1(ValidHandle, deleter.AsStdFunction());

	UniqueHandle<std::function<void(HANDLE)>> unique_handle2(std::move(unique_handle1));
...
TEST_F(TestUniqueHandle, MoveCtorFromInvalid) {
	MockFunction<void(HANDLE)> deleter;
	EXPECT_CALL(deleter, Call(_)).Times(0);
	UniqueHandle<std::function<void(HANDLE)>> unique_handle1(INVALID_HANDLE_VALUE, deleter.AsStdFunction());
...
TEST_F(TestUniqueHandle, MoveEqFromInvalidToValid) {
	...
	UniqueHandle<std::function<void(HANDLE)>> unique_handle1(INVALID_HANDLE_VALUE, deleter1.AsStdFunction());

	MockFunction<void(HANDLE)> deleter2;
	EXPECT_CALL(deleter2, Call(ValidHandle)).RetiresOnSaturation();
	UniqueHandle<std::function<void(HANDLE)>> unique_handle2(ValidHandle, deleter2.AsStdFunction());
